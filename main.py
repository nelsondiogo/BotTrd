...
def _reset_position_state():
    controller.state.update(
        current_position=None, entry_line=None, unrealized_pnl=0.0,
        peak_pnl_pct=0.0, lock_price=None, lock_stage_pct=0.0,
        opened_at=None, price_history=[], chart_symbol='BTCUSDT.P',
    )
        ...
        try:
            ...
            if now - last_balance_time > 20 or last_balance <= 0:
                try:
                    ...
                    if last_balance > 0 and abs(last_balance - tracker.session_start_balance) > 0.001:
                        add_log(f"BALANCE | {last_balance:.5f} USDT")
                    last_balance_time = now
                except Exception as e:
                    add_log(f"BALANCE ERR: {e}")
            ...
            for p in pairs:
                pairs_24h[p['symbol']] = {
                    'last': p['price'], 'high': p['price'] * 1.02,
                    'low': p['price'] * 0.98, 'change': p.get('change_24h', 0)
                }
    ...
    while controller.running:
        try:
            ...
            controller.state['price_history'].append({
                't': datetime.now().strftime('%H:%M:%S'), 'p': round(current_price, 8)
            })
            ...
            # SALDO ATUAL = INICIAL + REALIZADO + NAO REALIZADO
            current_bal = session_start + realized + unrealized

            total_pnl = realized + unrealized
            controller.state['pnl_history'].append({
                'time': datetime.now().strftime('%H:%M:%S'), 'pnl': round(total_pnl, 8)
            })
        ...
        try:
            ...
            if controller.state['consecutive_losses'] >= max_consecutive_losses:
                if not controller.state['circuit_breaker_triggered']:
                    add_log(f"CIRCUIT BREAKER | {controller.state['consecutive_losses']} perdas")
            ...
            if not tracker.symbol:
                ...
                # Stop global por drawdown
                drawdown_pct = (tracker.session_start_balance - total_bal) / tracker.session_start_balance * 100 if tracker.session_start_balance > 0 else 0
                max_drawdown = safe_float(settings.get('max_drawdown_pct', 15.0))
                if drawdown_pct >= max_drawdown:
                    add_log(f"STOP GLOBAL | Drawdown {drawdown_pct:.1f}%")
                ...
                if recent_losses >= 2 and any(t.get('symbol') == sym for t in recent_trades[-2:]):
                    add_log(f"BLOQUEIO | {sym} sequencia perdedora")
                    continue
                ...
                if qty <= 0 or (qty * price) < min_cost:
                    add_log(f"BLOQUEIO | Qty insuf {sym}")
                    continue
            ...
            if tracker.symbol and best_signal.symbol and best_signal.symbol != tracker.symbol:
                ...
                if opp_analyzer.should_rotate(tracker, best_signal, unrealized, current_price):
                    ...
                    try:
                        await ex.close_position(tracker.symbol, tracker.side, tracker.size)
                        await add_trade(tracker.symbol, tracker.side, tracker.entry_price, current_price, unrealized, 'ROTATE', tracker.leverage)
                        if unrealized < 0:
                            controller.state['consecutive_losses'] += 1
                        else:
                            controller.state['consecutive_losses'] = 0
...
async def trading_loop_async():
    ...
    try:
        ...
        total_bal = get_total_balance(bal)
        raw = bal.get('USDT', {})
        add_log(f"BALANCE RAW | total:{raw.get('total')} equity:{raw.get('equity')} avail:{raw.get('available')}")

        if total_bal <= 0:
            add_log("BALANCE ZERO - fallback 1.0")
            total_bal = 1.0
...
def api_bot_stop():
    ...
    # ZERA DASHBOARD COMPLETAMENTE
    _reset_position_state()
    controller.state.update(
        status='parado',
        realized_pnl=0.0,
        unrealized_pnl=0.0,
        current_balance=controller.state.get('session_start_balance', 0),
        last_signal='-',
        scan_results=[],
        active_pairs=[],
        pnl_history=[],
