...
# =============================================================================
# TASKS ASYNC - ARQUITETURA PARALELA HFT
# =============================================================================
async def scanner_task(ex, settings, tracker):
    """
    Market Scan INDEPENDENTE - roda a cada 15-20s mesmo com posição aberta.
    Usa asyncio.gather internamente via exchange_manager.fetch_ohlcv_batch.
    """
    scanner = MarketScanner(ex, settings)
    scanner_interval = 15  # 15s entre scans
...
async def monitor_task(ex, settings, tracker):
    """
    Monitora posição aberta com trailing stop agressivo.
    Roda a cada 0.8s para reação ultra-rápida.
    """
    ...
    while controller.running:
        try:
            ...
            # Calcula lock price para display
            lock_price = None
            ...
            # 2. Trailing Stop Agressivo (30%% do pico + lock profit)
...
async def executor_task(ex, settings, tracker):
    """
    Executor de ordens + Dynamic Opportunity Cost Analysis.
    Roda a cada 2s para decisões rápidas.
    """
    ...
    while controller.running:
        try:
            ...
            if not tracker.symbol:
                ...
                # Bloqueio por sequência perdedora no mesmo par
                ...
                if recent_losses >= 2 and any(t.get('symbol') == sym for t in recent_trades[-2:]):
                    add_log(f"BLOQUEIO | {sym} em sequência perdedora")
                    continue
            ...
            # ============ COM POSIÇÃO ABERTA - OPPORTUNITY COST ============
