    ...
    while controller.running:
        ...
        try:
            ...
            if error:
                ...
                add_log(f"SCAN [{latency}ms] | {len(pairs)} ops | {scanned} candidatos | TOP: {top_sym} ({top_score:.2f})")
...
async def executor_task(ex, settings, tracker):
    ...
    while controller.running:
        try:
            ...
            if not tracker.symbol:
                # Score mínimo 0.72 (era 0.82)
                if best_signal.score < 0.72:
                    continue
                # Permite confidence low para saldos baixos
                ...
                if qty <= 0 or (qty * price) < min_cost:
                    add_log(f"BLOQUEIO | Qty insuficiente {sym} | qty={qty:.6f} val={qty*price:.2f} min={min_cost}")
                    continue
...
async def trading_loop_async():
    ...
    try:
        ...
        bal = await ex.get_balance()
        total_bal = get_total_balance(bal)
        raw_info = bal.get('USDT', {}).get('raw_response', 'N/A')

        add_log(f"BALANCE RAW | {raw_info}")
