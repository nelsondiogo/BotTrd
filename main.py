...
<style>
  *{margin:0;padding:0;box-sizing:border-box;font-family:'Segoe UI',monospace;}
  ...
  input, select, textarea{background:#0f1318;border:1px solid #232a33;color:#e0e0e0;padding:8px 12px;border-radius:4px;width:100%;margin-bottom:8px;font-size:0.85rem;}
  ...
  .warning-box{background:#2d1b1b;border:1px solid #ff4757;border-radius:6px;padding:10px;margin-bottom:10px;font-size:0.8rem;color:#ff4757;}
  .info-box{background:#1b2d1b;border:1px solid #00d4aa;border-radius:6px;padding:10px;margin-bottom:10px;font-size:0.8rem;color:#00d4aa;}
</style>
</head>
<body>
<div class="container">
  ...
  <div class="grid">
    <div class="card" style="grid-column:span 2;">
      ...
      <div id="blocked-warning" class="warning-box" style="display:none;">
        ⚠️ <strong>IP Bloqueado pela Bybit!</strong> O servidor atual está em uma região bloqueada pelo CloudFront da Bybit.<br>
        Soluções: 1) Use <strong>Testnet</strong> (geralmente funciona) 2) Configure um <strong>Proxy SOCKS5</strong> abaixo
      </div>
      <div id="zero-balance-warning" class="warning-box" style="display:none;">
        ⚠️ <strong>Saldo USDT zero na Testnet!</strong> Você precisa depositar USDT de teste na sua conta Testnet.<br>
        <a href="https://testnet.bybit.com/app/assets/deposit" target="_blank" style="color:#00d4aa;">Clique aqui para depositar USDT de teste</a>
      </div>
      ...
      <div style="margin-top:10px;">
        <label style="font-size:0.75rem;color:#8892a0;">Proxy SOCKS5 (opcional - ex: socks5://user:pass@host:port)</label>
        <input type="text" id="cfg-proxy" placeholder="socks5://usuario:senha@host:porta (deixe vazio para sem proxy)">
      </div>
...
function updateUI(s) {
  ...
  if (s.status === 'RODANDO') badgeClass = 'status-running';
  else if (s.status === 'CIRCUIT_BREAKER') badgeClass = 'status-circuit';
  else if (s.status === 'BLOQUEADO') badgeClass = 'status-blocked';
  badge.className = 'status-badge ' + badgeClass;

  const blockedWarn = document.getElementById('blocked-warning');
  if (s.api_environment === 'blocked' || (s.api_message && s.api_message.includes('403'))) {
    blockedWarn.style.display = 'block';
  } else {
    blockedWarn.style.display = 'none';
  }

  const zeroWarn = document.getElementById('zero-balance-warning');
  if (s.api_message && s.api_message.includes('zero')) {
...
async function saveConfig() {
  const payload = {
    ...
    exchange: 'bybit',
    order_size_usdt: 0.9,
    proxy_url: document.getElementById('cfg-proxy').value.trim() || null
  };
...
async function loadConfig() {
  try {
    ...
    if (s.proxy_url) document.getElementById('cfg-proxy').value = s.proxy_url;
  } catch(e) {}
}
...
def api_credentials():
    ...
    settings['proxy_url'] = proxy.strip() if proxy else None
...
def api_start():
    ...
    async def _start():
        ...
        if not valid:
            if env == 'blocked':
                controller.state['status'] = 'BLOQUEADO'
            return False
        bal = await ex.get_balance()
        total = get_total_balance(bal)
        usdt_info = bal.get('USDT', {})

        # Log detalhado do saldo para debug
        add_log(f"BALANCE_RAW | wallet={usdt_info.get('wallet',0):.5f} equity={usdt_info.get('equity',0):.5f} available={usdt_info.get('available',0):.5f} total={total:.5f}")

        # Se saldo é zero mas API respondeu, pode ser Testnet sem fundos
        if total <= 0:
            if usdt_info.get('no_usdt'):
                add_log("ERRO: Nenhum USDT encontrado na conta. Deposite USDT de teste na Testnet.")
            else:
                add_log("ERRO: Saldo zero ou API sem permissão de leitura de saldo")
            return False
