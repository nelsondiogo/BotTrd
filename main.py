    ...
    while controller.running:
        ...
        try:
            ...
            _update_bot_state(
                ...
                scan_latency_ms=latency, current_balance=round(total_bal, 8),
            )

            if int(now) % 20 < 2:
                await save_bot_state({
                    'session_start_balance': controller.state.get('session_start_balance', 0),
                    'scan_results': pairs, 'last_scan': datetime.now().isoformat(),
...
async def executor_task(ex, settings, tracker):
    ...
    while controller.running:
        try:
            ...
            if not tracker.symbol:
                if best_signal.score < 0.82: continue
                if best_signal.confidence == 'low': continue
...
async def trading_loop_async():
    ...
    try:
        ...
        if total_bal <= 0:
            add_log("BALANCE ZERO - verifique API permissions (Read-Write necessario para wallet)")
            total_bal = 1.0
        ...
        add_log(f"START HFT | Balance inicial: {total_bal:.5f} USDT | SL: {settings.get('stop_loss_pct', 1.5)}%")
