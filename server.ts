...
async function tradingLoop() {
  ...
  try {
    ...
    // Gestão de Posição Ativa (Trailing Stop & Time Stop)
    if (activePos) {
    ...
    // Varredura de Mercado (Scanner)
    const scanResults = await Promise.all(botStatus.activePairs.map(async (p: any) => {
    ...
    if (validSignals.length > 0 && !activePos) {
      const bestSignal = validSignals[0]!;
      addLog(`Entrada Detectada: ${bestSignal.symbol} (Score: ${bestSignal.score.toFixed(2)})`);
    ...
    addLog(`Erro no Ciclo: ${error instanceof Error ? error.message : String(error)}`);
  } finally {
  ...
  try {
    ...
    app.post("/api/close", async (req, res) => {
      try {
        ...
        } else res.status(400).json({ error: "Sem posição." });
      } catch (e) { res.status(500).json({ error: "Falha ao fechar." }); }
    });
    ...
    console.error("[CRITICAL] Falha ao iniciar:", err);
    process.exit(1);
  }
}
