...
class ExchangeManager:
    ...
    async def get_balance(self) -> Dict:
        ...
        Tenta múltiplos campos e endpoints para garantir leitura do saldo.
        """
        # Endpoint 1: wallet-balance UNIFIED
        ...
        if data.get('retCode') == 0:
            ...
            for acc in accounts:
                # Tenta totalWalletBalance primeiro (nível da conta)
                total_wallet = acc.get('totalWalletBalance')
                total_equity = acc.get('totalEquity')

                # Itera coins para achar USDT
                coins = acc.get('coin', [])
                usdt_wallet = 0.0
                usdt_equity = 0.0
                usdt_available = 0.0
                ...
                for coin in coins:
                    if coin.get('coin') == 'USDT':
                        usdt_wallet = float(coin.get('walletBalance', 0) or 0)
                        usdt_equity = float(coin.get('equity', 0) or 0)
                        usdt_available = float(coin.get('availableToWithdraw', 0) or 0)
                        break

                # Usa o maior valor disponível
                final_total = max(
                    usdt_wallet, usdt_equity,
                    float(total_wallet or 0),
                    float(total_equity or 0)
                )
                ...
                return {
                    'USDT': {
                        ...
                        'raw_response': f"wallet={usdt_wallet} equity={usdt_equity} totalWallet={total_wallet} totalEquity={total_equity}"
                    }
                }

        # Se falhou, tenta endpoint alternativo (all-balance)
        data2 = await self._request('GET', '/v5/asset/transfer/query-account-coins-balance?memberId=&accountType=UNIFIED&coin=USDT', signed=True)
        if data2.get('retCode') == 0:
            result2 = data2.get('result', {})
            balance = result2.get('balance', [])
            for b in balance:
                if b.get('coin') == 'USDT':
                    wallet = float(b.get('walletBalance', 0) or 0)
                    transfer = float(b.get('transferBalance', 0) or 0)
                    return {
                        'USDT': {
                            'total': max(wallet, transfer),
                            'wallet': wallet,
                            'equity': transfer,
                            'available': transfer,
                            'raw_response': f"transfer_endpoint wallet={wallet} transfer={transfer}"
                        }
                    }

        return {
            'USDT': {
                'total': 0.0, 'wallet': 0.0, 'equity': 0.0, 'available': 0.0,
                'raw_response': f"retCode={data.get('retCode')} msg={data.get('retMsg')}"
            }
        }
