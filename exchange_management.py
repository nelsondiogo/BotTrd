...
class ExchangeManager:
    ...
    async def get_balance(self) -> Dict:
        ...
        Estrutura real: result.list[0].coin[] ou result.list[0].totalWalletBalance
        """
        ...
        if data.get('retCode') == 0:
            ...
            for acc in accounts:
                # Tenta pegar totalWalletBalance do nível da conta primeiro
                total_wallet = acc.get('totalWalletBalance')
                total_equity = acc.get('totalEquity')

                # Itera coins para achar USDT especificamente
                coins = acc.get('coin', [])
                usdt_wallet = 0.0
                usdt_equity = 0.0
                usdt_available = 0.0
                ...
                # Usa o maior valor disponível: coin USDT > totalWalletBalance > totalEquity
                final_total = max(usdt_wallet, usdt_equity, float(total_wallet or 0), float(total_equity or 0))
