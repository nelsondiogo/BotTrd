...
class MarketScanner:
    ...
    async def scan(self, balance: float, top_n: int = 7, timeframe: str = '15') -> Dict[str, Any]:
        ...
        # Fetch paralelo com asyncio.gather via exchange_manager
        try:
        ...
        # Ordena por score, depois por expected_return
    ...
    async def _filter_candidates(self, tickers: Dict, balance: float) -> List[Dict]:
        ...
        for symbol, ticker in tickers.items():
            ...
            if last_price <= 0 or volume <= 0:
                continue
            if volume < 2_000_000:  # Volume mínimo mais alto para HFT
                continue
            ...
            if bid > 0 and ask > 0:
                ...
                if spread_pct > 0.08:  # Spread mais apertado para HFT
                    continue
        ...
        top_candidates = candidates[:25]  # Top 25 para análise mais profunda
