...
class MarketScanner:
    ...
    async def scan(self, balance: float, top_n: int = 7, timeframe: str = '15') -> Dict[str, Any]:
        ...
        try:
            ...
            if isinstance(ohlcv_m15_map, Exception): raise ohlcv_m15_map
            if isinstance(ohlcv_h1_map, Exception): raise ohlcv_h1_map
            if isinstance(ohlcv_h4_map, Exception): ohlcv_h4_map = {}
        except Exception as e:
            return {'pairs': [], 'count': 0, 'latency_ms': 0, 'error': str(e)}
        ...
        for c in candidates:
            ...
            if not ohlcv_m15 or not ohlcv_h1: continue
            if len(ohlcv_m15) < 35 or len(ohlcv_h1) < 25: continue
            try:
                sig = self.signal_gen.generate(ohlcv_m15, ohlcv_h1, ohlcv_h4)
                if sig.side:
                    results.append({
                        'symbol': sym, 'price': c['price'], 'score': sig.score,
                        'side': sig.side, 'confidence': sig.confidence,
                        'expected_return_pct': sig.expected_return_pct,
                        'volume_24h': c['volume'], 'change_24h': round(c['change'], 2),
                        'adx': sig.meta.get('adx', 0), 'rsi': sig.meta.get('rsi', 0),
                        'mfi': sig.meta.get('mfi', 0), 'trend_h1': sig.meta.get('trend_h1', 'NEUTRO'),
                        'h1_adx': sig.meta.get('h1_adx', 0), 'atr': sig.meta.get('atr', 0),
        ...
        return {'pairs': top, 'count': len(top), 'latency_ms': latency_ms, 'scanned': len(candidates), 'error': None}
    ...
    async def _filter_candidates(self, tickers: Dict, balance: float) -> List[Dict]:
        ...
        for symbol, ticker in tickers.items():
            ...
            if last_price <= 0 or volume <= 0: continue

            # Volume mínimo adaptativo: saldo baixo = menos restrição
            min_volume = 100_000 if balance < 5 else 500_000
            ...
            if bid > 0 and ask > 0:
                ...
                if spread_pct > 0.2: continue

            candidates.append({
                'symbol': symbol, 'price': last_price, 'volume': volume,
                'change': float(ticker.get('percentage', 0) or 0), 'spread_pct': spread_pct,
            })

        candidates.sort(key=lambda x: x['volume'], reverse=True)
        # Para saldos baixos, analisa mais pares para ter chance de achar sinais
        top_n = 40 if balance < 5 else 30
        self._candidates_cache = candidates[:top_n]
    ...
    async def _fetch_ohlcv_cached(self, symbols: List[str], timeframe: str, limit: int) -> Dict[str, Any]:
        ...
        if symbols_to_fetch:
            fetched = await self.ex.fetch_ohlcv_batch(symbols_to_fetch, timeframe=timeframe, limit=limit, max_concurrent=8)
            for sym, data in fetched.items():
                if data:
                    self._ohlcv_cache[f"{sym}_{timeframe}"] = {'data': data, 'time': now}
                    result[sym] = data
        return result
