...
class MarketScanner:
    def __init__(self, exchange_manager, settings: Dict[str, Any]):
        ...
        self._candidates_cache_time = 0
        self._candidates_ttl = 60
    ...
    async def scan(self, balance: float, top_n: int = 7, timeframe: str = '15') -> Dict[str, Any]:
        ...
        try:
            ...
            ohlcv_h1_map = await self._fetch_ohlcv_cached(symbols, timeframe='60', limit=30)
            ohlcv_h4_map = await self._fetch_ohlcv_cached(symbols, timeframe='240', limit=20)
        except Exception as e:
            return {'pairs': [], 'count': 0, 'latency_ms': 0, 'error': str(e)}
