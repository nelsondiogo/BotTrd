...
class MarketScanner:
    ...
    async def _filter_candidates(self, tickers: Dict, balance: float) -> List[Dict]:
        ...
        for symbol, ticker in tickers.items():
            ...
            if volume < 300_000: continue  # Reduzido para saldo baixo

            # Só filtra por min_cost se o saldo for razoavelmente alto (> 10 USDT)
            # Com saldo baixo, deixa o risk manager decidir o tamanho depois
            if balance >= 10:
                min_cost = self.ex.get_min_cost(symbol, last_price)
                if min_cost > balance * 0.9:
                    continue
        ...
        self._candidates_cache = candidates[:35]
