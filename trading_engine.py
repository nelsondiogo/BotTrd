...
class SignalGenerator:
    ...
    def generate(self, ohlcv_m15: List[List[float]], ohlcv_h1: List[List[float]], ohlcv_h4: Optional[List[List[float]]] = None) -> SignalResult:
        ...
        if side and score >= 0.80:
            confidence = 'high' if score >= 0.88 else 'medium' if score >= 0.84 else 'low'

            # Filtro extra: H4 deve confirmar tendência se disponível
            if ohlcv_h4 and len(ohlcv_h4) >= 22:
                ...
                if side == 'long' and h4_ema9 <= h4_ema21:
                    return SignalResult()  # H4 não confirma alta
                if side == 'short' and h4_ema9 >= h4_ema21:
                    return SignalResult()  # H4 não confirma baixa
...
class PositionTracker:
    ...
    def _get_drawdown_threshold(self, peak: float) -> float:
        """Stop quando perder 15% do lucro obtido (do pico)."""
        if peak > 0:
            return peak * 0.85  # Deixa escapar no máximo 15% do lucro de pico
        return 0.0
    ...
    def check_trailing_stop(self, current_price: float) -> tuple[bool, str]:
        ...
        if self.peak_profit_pct > 0:
            ...
            if profit <= threshold:
                return True, f"DRAWDOWN_15% | peak={self.peak_profit_pct:.2f}% current={profit:.2f}%"
