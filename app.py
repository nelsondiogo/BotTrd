...
from history_manager import add_trade, get_history, get_stats, save_bot_state, load_bot_state, clear_bot_state, get_recent_trades
...
def get_total_balance(balance_obj):
    ...
    return safe_float(usdt.get('total'), 0.0) if usdt else 0.0
...
def _reset_position_state():
    """Limpa todo o estado da posição no dashboard."""
    _update_bot_state(
        current_position=None,
        entry_line=None,
        unrealized_pnl=0.0,
...
async def _close_and_record(ex, tracker, reason='MANUAL'):
    ...
    try:
        ...
        if pos and safe_float(pos.get('contracts'), 0) > 0:
            ...
            realized = safe_float(bot_state.get('realized_pnl'), 0) + unrealized
...
async def scanner_task(ex, settings, tracker):
    """Task de scanner otimizada com cache e intervalo reduzido."""
    scanner = MarketScanner(ex, settings)
    scanner_interval = 3  # REDUZIDO de 18 para 3 segundos

    # Cache de saldo (atualizado a cada 30s)
    last_balance = 0.0
    last_balance_time = 0
    ...
    while bot_running:
        ...
        try:
            ...
            res = await asyncio.wait_for(
                scanner.scan(balance=total_bal, top_n=7, timeframe=settings.get('timeframe', '15m')),
                timeout=15.0
            )
            ...
            # Dados de 24h (reais se disponíveis)
            pairs_24h = {}
            for p in pairs:
                pairs_24h[p['symbol']] = {
                    'last': p['price'],
                    'high': p.get('high_24h', p['price'] * 1.02),
                    'low': p.get('low_24h', p['price'] * 0.98),
            ...
            # Salva estado a cada 15s (não a cada scan)
            if int(now) % 15 < 3:
                await save_bot_state({
                    'session_start_balance': bot_state.get('session_start_balance', 0),
                    'scan_results': pairs,
        ...
        # Sleep dinâmico para manter intervalo consistente
...
async def monitor_task(ex, settings, tracker):
    """Task de monitoramento com trailing stop sempre ativo."""
    ...
    while bot_running:
        try:
            ...
            # Histórico de preços: 300 pontos = 5 minutos (sleep=1)
            bot_state['price_history'].append({
                't': datetime.now().strftime('%H:%M:%S'),
                'p': round(current_price, 8)
            })
            if len(bot_state['price_history']) > 300:
                bot_state['price_history'].pop(0)
            ...
            session_start = safe_float(bot_state.get('session_start_balance'), 0)
            realized = safe_float(bot_state.get('realized_pnl'), 0)
            ...
            # TRAILING STOP: SEMPRE ativo (não só com unrealized > 0)
            ...
            # Histórico de PNL: 600 pontos = 10 minutos
            total_pnl = realized + unrealized
            bot_state['pnl_history'].append({
                'time': datetime.now().strftime('%H:%M:%S'),
                'pnl': round(total_pnl, 8)
            })
            if len(bot_state['pnl_history']) > 600:
...
async def executor_task(ex, settings, tracker):
    """Task de execução com critérios rigorosos e circuit breaker."""
    ...
    while bot_running:
        try:
            ...
            # CIRCUIT BREAKER: verifica antes de qualquer ação
            if bot_state['consecutive_losses'] >= max_consecutive_losses:
                if not bot_state['circuit_breaker_triggered']:
                    add_log(f"CIRCUIT BREAKER | {bot_state['consecutive_losses']} perdas consecutivas. Bot pausado.")
                    bot_state['circuit_breaker_triggered'] = True
                    bot_state['status'] = 'CIRCUIT_BREAKER'
                await asyncio.sleep(5)
                continue

            scan_results = list(bot_state.get('scan_results', []))
            ...
            # SEM POSIÇÃO: verifica se pode abrir
            if not tracker.symbol:
                # Score mínimo aumentado para 0.72
                if best_signal.score < 0.72:
                    continue

                # Só entra se confiança for medium ou high
                ...
                # Verifica se não está repetindo par perdedor recente
                ...
                # Verifica slippage com preço atual
                try:
                    ...
                    if slip_pct > 0.30:
                        add_log(f"BLOQUEIO | Slippage {slip_pct:.2f}% > 0.30% em {sym}")
                        continue
                    price = current_price  # Atualiza preço de entrada
                except Exception:
                    pass
            ...
            if tracker.symbol and best_signal.symbol and best_signal.symbol != tracker.symbol:
                ...
                if opp_analyzer.should_rotate(tracker, best_signal, unrealized, current_price):
                    ...
                    try:
                        ...
                        _update_bot_state(realized_pnl=round(safe_float(bot_state.get('realized_pnl'), 0) + unrealized, 8))
                        _reset_position_state()
                        tracker.reset()
                        await asyncio.sleep(2)
                    except Exception as e:
...
async def health_check_task(ex):
    """Task de health check para manter conexão ativa."""
    while bot_running:
        try:
            await asyncio.sleep(60)
            # Ping na exchange para manter conexão viva
            await ex.fetch_ticker('BTCUSDT:USDT')
        except Exception as e:
            add_log(f"HEALTH CHECK: {e}")


async def trading_loop_async():
    """Loop principal de trading — todas as tasks async."""
    global bot_running, _ex_global, _tracker_global
    settings = load_settings()

    # Usa apenas ExchangeManager async
    ex = ExchangeManager(
    ...
    try:
        ...
        total_bal = get_total_balance(bal)
        if total_bal <= 0:
            add_log("SALDO ZERO - usando 1.0 como referência")
            total_bal = 1.0

        tracker = PositionTracker()
        tracker.session_start_balance = total_bal
        _tracker_global = tracker
        ...
        add_log(f"START | Saldo: {total_bal:.5f} USDT")

        # Cria todas as tasks async (SEM thread de watchdog!)
        tasks = [
            asyncio.create_task(scanner_task(ex, settings, tracker)),
            asyncio.create_task(monitor_task(ex, settings, tracker)),
            asyncio.create_task(executor_task(ex, settings, tracker)),
...
def api_bot_start():
    ...
    bot_running = True
    bot_state['status'] = 'iniciando'
    bot_state['logs'] = []
    bot_state['consecutive_losses'] = 0
    bot_state['circuit_breaker_triggered'] = False
...
def api_bot_stop():
    ...
    if _tracker_global and _tracker_global.symbol:
        async def _close():
            await _close_and_record(_ex_global, _tracker_global, 'MANUAL_STOP')
